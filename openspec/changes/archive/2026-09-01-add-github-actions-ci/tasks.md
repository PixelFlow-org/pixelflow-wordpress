...
- [x] 4.4 **[M]** Pushed, opened and merged as PR #15

## 5. Between the pull requests — provision the registry credential

- [x] 5.1 **[M]** Classic PAT with `read:packages` created
- [x] 5.2 **[M]** Saved as repository secret `GH_PACKAGES_TOKEN`; proven to work on the first run (frontend installed in 26s)
...
- [x] 7.4 **[M]** Merged as PR #16 with all five checks green

## 8. Manual step — enable protection

Maintainer action in the GitHub UI, after group 7 has merged.

> **BLOCKED — not on the maintainer, on repository permissions.** The maintainer's
> account has Write, not Admin, on `PixelFlow-org/pixelflow-wordpress`: the repository
> Settings page reports "You don't have access to repository options" and exposes only
> Secrets and variables. Rulesets can be created only by an organisation owner or by
> someone granted Admin on the repository. `.github/BRANCH_PROTECTION.md` is written to
> be handed over as-is — it carries the full ruleset, the single required check name and
> the emergency procedure.
>
> Consequence while this is open: the pipeline runs and reports on every pull request,
> but nothing enforces it. A red check does not block a merge, and `main` still accepts
> direct pushes.
...
- [ ] 8.2 **[M, BLOCKED: needs Admin]** Apply the ruleset from `.github/BRANCH_PROTECTION.md`
- [ ] 8.3 **[M, BLOCKED: needs Admin]** Verify protection: a direct push to `main` is rejected, and a pull request carrying a deliberate lint error cannot be merged
- [ ] 8.4 **[M, BLOCKED: needs Admin]** Verify the escape hatch: confirm the documented disable procedure in 7.1 matches the UI
